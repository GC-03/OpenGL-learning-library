/*
* This program contains the basics of OpenGL, and uses its foundamental concepts to draw a triangle.
* Note that the triangle will squeeze and stretch when the window is resized because the coordinates are not normalized,
* so they will just translate to window coordinates before rasterization.
* 
...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		//Drawing the object
		glDrawArrays(GL_TRIANGLES, 0, 3);																							//Draws what the vertices found in the currently active shader and the currently bound vao (or vbo + configuration)
